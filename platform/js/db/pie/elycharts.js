...
$.elycharts.tooltipmanager = {
  ...
  onMouseEnter : function(env, serie, index, mouseAreaData) {
    ...
    if (props.width && props.width != 'auto' && props.height && props.height != 'auto') {
      ...
      env.tooltipContainer.width(props.width + delta + 50).height(props.height + delta + 50);
      if (!env.tooltipFrameElement && props.frameProps)
        env.tooltipFrameElement = env.tooltipFrame.rect(delta / 2, delta / 2, props.width+20, props.height+20, props.roundedCorners);
    }
...
$.elycharts.pie = {
  ...
  draw : function(env) {
    ...
    //var r = env.opt.r ? env.opt.r : Math.floor((w < h ? w : h) / 3);
	var r = Math.floor((w < h?w:h)*0.8/3);
